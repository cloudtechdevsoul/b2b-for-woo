	...
	class Product_Sales_Report_Table extends WP_List_Table {
		...
		public function column_default( $product_id, $column_name ) {
			...
			switch ($column_name) {
				...
				case 'price':
					return wp_kses(str_replace(',', '', wc_price($product->get_price())),wp_kses_allowed_html('post'));
				case 'total_tax':
					$this->sum_of_tax_total += $total_tax;
					return wp_kses(str_replace(',', '', wc_price($total_tax)),wp_kses_allowed_html('post'));
				case 'subtotal':
					$this->sum_of_subtotal += $sub_total;
					return wp_kses(str_replace(',', '', wc_price($sub_total)),wp_kses_allowed_html('post'));
				case 'total':
					$this->sum_of_total += $total;
					return wp_kses(str_replace(',', '', wc_price($total)),wp_kses_allowed_html('post'));
				case 'action':
					ob_start();
			...
			echo '<div class="ct-psbsp-total-of-selected-table">
                <table>
                    <tr class="ct-psbsp-subtotal">
                        ...
                        <td class="ct-psbsp-subtotal-td">' . wp_kses(str_replace(',', '', wc_price($this->sum_of_subtotal)),wp_kses_allowed_html('post')) . '</td>
                    </tr>
                    <tr class="ct-psbsp-total-tax">
                        <th>' . esc_html__('Total Tax', 'cloud_tech_psbsp') . '</th>
                        <td class="ct-psbsp-total-tax-td">' . wp_kses(str_replace(',', '', wc_price($this->sum_of_tax_total)),wp_kses_allowed_html('post')) . '</td>
                    </tr>
                    <tr class="ct-psbsp-total">
                        <th>' . esc_html__('Total', 'cloud_tech_psbsp') . '</th>
                        <td class="ct-psbsp-total-td">' . wp_kses(str_replace(',', '', wc_price($this->sum_of_total)),wp_kses_allowed_html('post')) . '</td>
                    </tr>
                </table>
            </div>';
		}
