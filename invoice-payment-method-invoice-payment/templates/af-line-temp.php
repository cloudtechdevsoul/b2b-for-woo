	...
	<div class="af-pdf-wrapper">
				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<?php } ?>	
							<td class="product_sku_td"><span><?php echo wp_kses( $product_name ,wp_kses_allowed_html('post')); ?></span><br>
								<?php
								if ( $af_sku ) {
									?>
									<small><?php echo wp_kses( $sku ,wp_kses_allowed_html('post')); ?></small><br>
									<?php
								}
								?>
							</td>
							<?php if ( $af_quantity ) { ?>
								<td><?php echo wp_kses( $quantity ,wp_kses_allowed_html('post')); ?></td>
							<?php } ?>

							<?php if ( $af_price ) { ?>
								<td><?php echo wp_kses( wc_price( $price_html ) ,wp_kses_allowed_html('post')); ?></td>
							<?php } ?>
							<?php if ( $af_regular_price ) { ?>
								<td><?php echo wp_kses( wc_price( $regular_price ) ,wp_kses_allowed_html('post')); ?></td>
							<?php } ?>
							<?php if ( $af_on_sale && ! empty( $sale_price ) ) { ?>
								<td><?php echo wp_kses( wc_price( $sale_price ) ,wp_kses_allowed_html('post')); ?></td>
							<?php } ?>
							<?php if ( $af_total ) { ?>
								<td><?php echo wp_kses( wc_price( $total ) ,wp_kses_allowed_html('post')); ?></td>
							<?php } ?>
							<?php if ( $af_tax ) { ?>
								<td><?php echo wp_kses( wc_price( $af_tax1 ) ,wp_kses_allowed_html('post')); ?></td>
							<?php } ?>
							<?php if ( $af_toatl_inc_taxt ) { ?>
								<td><?php echo wp_kses( wc_price( $total_inc_taxt ) ,wp_kses_allowed_html('post')); ?></td>
							<?php } ?>
							<?php if ( $af_line_total ) { ?>
								<td><?php echo wp_kses( wc_price( $af_line_total ) ,wp_kses_allowed_html('post')); ?></td>
							<?php } ?>
						</tr>
							<?php
						}//end foreach
		...
		<section class="af-subtotal-section">
			...
			<div class="af-invoice-subtotal-box">
				<ul>
					<li><p><label><?php esc_html_e( 'Subtotal', 'af_ips_invoices' ); ?></label><font><?php echo wp_kses( wc_price( $af_subtotal ) ,wp_kses_allowed_html('post')); ?></font></p></li>
					<li><p><label><?php esc_html_e( 'Discount', 'af_ips_invoices' ); ?></label><font><?php echo wp_kses( wc_price( $af_discount ) ,wp_kses_allowed_html('post')); ?></font></p></li>
					<?php if ( $order->get_shipping_total() ) { ?>
						<li><p><label><?php echo esc_attr( $order->get_shipping_method() ); ?></label><font><?php echo wp_kses( wc_price( $order->get_shipping_total() ) ,wp_kses_allowed_html('post')); ?></font></p></li>
					<?php } ?>
					<li><p class="af-total-label"><label><?php esc_html_e( 'Total', 'af_ips_invoices' ); ?></label><font><?php echo wp_kses( wc_price( $af_cart_total ) ,wp_kses_allowed_html('post')); ?></font></p></li>
				</ul>
			</div>
		</section>
